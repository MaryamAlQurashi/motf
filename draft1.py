...
class MOTFSniffer():
    ...
    def parse_packet_tcp_layer(self,packet):
        ...
        packet_data = {
            ...
            'HTTP_VRS': str(packet[HTTPResponse].Http_Version) if packet.haslayer(HTTPResponse) else None,
            'HTTP_VRQ': str(packet[HTTPRequest].Http_Version) if packet.haslayer(HTTPRequest) else None,
            ...
            'CONT-LEN-RS': str(packet[HTTPResponse].Content_Length) if packet.haslayer(HTTPResponse) else None,
            'CONT-LEN-RQ': str(packet[HTTPRequest].Content_Length) if packet.haslayer(HTTPRequest) else None,
            'CONT-TYPE-RS': str(packet[HTTPResponse].Content_Type) if packet.haslayer(HTTPResponse)  else None,
            'CONT-TYPE-RQ': str(packet[HTTPRequest].Content_Type) if packet.haslayer(HTTPRequest)  else None,
